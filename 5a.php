...
$query = "SELECT angajati.salariu
FROM Angajati JOIN Certificare ON angajati.idan = certificare.idan
              JOIN Aeronave ON certificare.idav = aeronave.idav
WHERE angajati.salariu IN(SELECT salariu FROM angajati WHERE functie = 'pilot') AND Aeronave.numeav LIKE '".$caractere."%'
ORDER BY angajati.salariu";
...
for ($i=0; $i < $num_results; $i++)
{
    ...
    echo '<td align="center">'.htmlspecialchars(stripslashes($row['salariu'])).'</td>';
	$i=$num_results;
